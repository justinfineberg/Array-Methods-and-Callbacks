...
function getYears(arr, cbfunction) {
    let first = cbfunction(arr);
    let years = first.map(function(item){
        return item['Year'];
    })
    return years;
}
...
function getWinners(arr, cbfunction) {
    let workingArray = cbfunction(arr);
    let winners = [];
...
function getWinnersByYear(arr, cbfunction1, cbfunction2) {
    let answer = [];
    let years = cbfunction1(arr)
    let names = cbfunction2(arr)
    for (let i = 0; i < years.length; i++){
        answer.push(`In ${years[i]}, ${names[i]} won the world cup!`)
    }
    return answer;
}
...
function getAverageGoals(cbfunction1) {
   let arr = cbfunction1
   let num = arr.reduce(function(acc, item){
